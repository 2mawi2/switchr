...
impl Project {
    ...
    pub fn new_github<P: Into<PathBuf>>(
        name: String,
        path: P,
        github_url: String,
    ) -> Self {
    ...
    pub fn display_string(&self) -> String {
        ...
        format!("{} {}{} - {}", source_indicator, self.name, time_str, self.path.display())
    }
}
...
impl ProjectList {
    ...
    pub fn deduplicate(&mut self) {
        ...
        let local_paths: std::collections::HashSet<_> = self.projects
            .iter()
...
mod tests {
    ...
    fn test_display_string() {
        ...
        let local_project = Project::new_local("local-proj".to_string(), "/path");
        assert!(local_project.display_string().contains("📁"));
        assert!(local_project.display_string().contains("local-proj"));
        ...
        let display = github_project.display_string();
        assert!(display.contains("🐙"));
        assert!(display.contains("gh-proj"));
    ...
    fn test_project_list_sorting() {
        ...
        let old_project = Project::new_local("old".to_string(), "/old")
            .with_last_modified(old_time);
        let new_project = Project::new_local("new".to_string(), "/new")
    ...
    fn test_filter_by_source() {
        let projects = vec![
            ...
            Project::new_github("github1".to_string(), "/path3", "https://github.com/user/repo".to_string()),
        ];
    ...
    fn test_deduplicate_projects() {
        ...
        let projects = vec![
            ...
            Project::new_github("my-project".to_string(), shared_path.clone(), "https://github.com/user/my-project".to_string()),
        ...
        let remaining_projects: Vec<_> = project_list.projects().iter().map(|p| (&p.name, &p.source)).collect();
        assert!(remaining_projects.contains(&(&"my-project".to_string(), &ProjectSource::Local)));
        assert!(remaining_projects.contains(&(&"other-project".to_string(), &ProjectSource::Cursor)));
        assert!(!remaining_projects.iter().any(|(_, source)| **source == ProjectSource::GitHub));
    }
    ...
    fn test_deduplicate_no_duplicates() {
        let projects = vec![
            ...
            Project::new_github("project3".to_string(), "/path3", "https://github.com/user/project3".to_string()),
        ];
