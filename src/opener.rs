...
impl ProjectOpener {
    ...
    fn clone_github_project(&self, project: &Project) -> Result<()> {
        let github_url = project.github_url.as_ref()
        ...
        println!("Successfully cloned {} to {}", project.name, project.path.display());
        Ok(())
    }
    ...
    pub fn open_project_path<P: AsRef<Path>>(&self, path: P, config: &Config) -> Result<()> {
        ...
        if is_background_editor(editor) {
            ...
            let output = cmd.output()
                .with_context(|| format!("Failed to execute editor command: {}", config.editor_command))?;
...
mod tests {
    ...
    fn test_open_with_whitespace_only_editor_command() {
        ...
        let result = opener.open_project(&project, &config);
        assert!(result.is_err(), "Should fail with whitespace-only editor command");
        assert!(result.unwrap_err().to_string().contains("empty"));
    }
