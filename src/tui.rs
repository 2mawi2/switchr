...
use ratatui::{
    ...
    text::{Line, Span, Text},
    widgets::{Block, Borders, BorderType, List, ListItem, Paragraph, Padding},
    Frame, Terminal,
};
use std::io;
...
impl TuiApp {
    pub fn new(projects: Vec<Project>) -> Self {
        ...
        let project_exists_cache: Vec<bool> = projects.iter()
    ...
    pub fn run_interactive<B: Backend>(projects: Vec<Project>, terminal: &mut Terminal<B>) -> Result<Option<Project>> {
    ...
    fn update_filtered_projects(&mut self) {
        if self.input.is_empty() {
            ...
            let mut scored: Vec<(usize, i64)> = self.projects
                .iter()
                .enumerate()
    ...
    fn compute_github_status(projects: &[Project]) -> String {
        ...
        let has_github_projects = projects.iter().any(|p| p.source == crate::models::ProjectSource::GitHub);
    ...
    fn draw(&self, f: &mut Frame) {
        ...
        let search_content = if self.input.is_empty() {
            Text::from(vec![
                ...
                Line::from(vec![
                    Span::styled(search_placeholder, Style::default().fg(TEXT_MUTED).italic()),
                ]),
            ])
        } else {
            Text::from(vec![
        ...
        let search_box = Paragraph::new(search_content)
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
                    .border_style(Style::default().fg(if self.input.is_empty() { TEXT_MUTED } else { PRIMARY_COLOR }))
                    .title(Line::from(vec![
                        Span::styled(" ", Style::default()),
                        Span::styled("Search", Style::default().fg(TEXT_PRIMARY).add_modifier(Modifier::BOLD)),
                        Span::styled(" ", Style::default()),
                    ]))
        ...
        let items: Vec<ListItem> = self.filtered_projects
            .iter()
            .enumerate()
            .map(|(i, (project_index, _score))| {
                ...
                if is_selected {
                    line_spans.extend(vec![
                        Span::styled(" ", Style::default()),
                        Span::styled(format!("[{}]", source_label), Style::default()
                            .fg(source_color)
        ...
        let projects_title = format!(" Projects ({}/{}) ", 
        ...
        let projects_list = List::new(items)
            .block(
                Block::default()
                    ...
                    .title(Line::from(vec![
                        Span::styled(projects_title, Style::default()
                            .fg(TEXT_PRIMARY)
                            .add_modifier(Modifier::BOLD)),
                    ]))
        ...
        let status_content = Text::from(vec![
            Line::from(vec![
                ...
                Span::styled(format!("{}", self.projects.len()), Style::default().fg(ACCENT_COLOR).add_modifier(Modifier::BOLD)),
                Span::styled("  │  ", Style::default().fg(TEXT_MUTED)),
                Span::styled("🔍 Shown: ", Style::default().fg(TEXT_SECONDARY)),
                Span::styled(format!("{}", self.filtered_projects.len()), Style::default().fg(PRIMARY_COLOR).add_modifier(Modifier::BOLD)),
            ]),
        ]);
        ...
        let help_content = Text::from(vec![
            Line::from(vec![
                ...
                Span::styled("Esc/q", Style::default().fg(ERROR_COLOR).add_modifier(Modifier::BOLD)),
...
mod tests {
    use super::*;
    use crate::models::{ProjectSource, Project};
